...
namespace System {
	...
	partial class TimeZoneInfo {
		...
		static class AndroidTimeZones {
			...
			static int GetSystemProperty (string name, ref IntPtr value)
			{
				unsafe {
					fixed (byte *name_p = Encoding.UTF8.GetBytes(name)) {
						return GetSystemProperty(name_p, ref value);
					}
				}
			}

			[MethodImplAttribute (MethodImplOptions.InternalCall)]
			static unsafe extern int GetSystemProperty (byte *name, ref IntPtr value);
